...
OUTDIR = ROOT / "solutions" / "phase_a" / "phase_a_singularity_structure"
...
def write_candidate_factor_check() -> Dict[str, object]:
    values_65 = [k * math.pi / 32.0 for k in range(-32, 33)]
    values_17 = [k * math.pi / 8.0 for k in range(-8, 9)]
    rows = []
    max_abs_err = 0.0

    for theta in values_65:
        for phi in values_65:
            for rho in values_17:
                detj = det4(jacobian(0.0, theta, phi, rho))
    ...
    with (OUTDIR / "candidate_factor_check.csv").open("w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=list(rows[0].keys()))
        writer.writeheader()
        writer.writerows(rows)

    return {"rows": len(rows), "max_abs_err": max_abs_err}


def write_phi_reference_table() -> List[Dict[str, float]]:
    phis = [k * math.pi / 4.0 for k in range(-4, 5)]
    rows = []
    for phi in phis:
        rows.append({
            "phi": phi,
            "detJ": det4(jacobian(0.0, 0.0, phi, 0.0)),
            "cos2phi": math.cos(2.0 * phi),
        })
    with (OUTDIR / "phi_reference_table.json").open("w") as f:
...
def write_summary(
    ...
    candidate_check: Dict[str, object],
    phi_reference: List[Dict[str, float]],
) -> None:
    lines = []
    lines.append("# Phase A Singularity Structure Summary")
    lines.append("")
    lines.append("Generated by `analysis/phase_a/phase_a_singularity_structure.py`.")
    lines.append("")
    ...
    lines.append("## Method")
    lines.append("")
    lines.append("- Evaluate the exact Jacobian on the full active-domain scan used for Phase A closure.")
    lines.append("- Compare det J against the candidate factorization e^(4 omega) cos(2 phi) across the active box.")
    lines.append("- Save explicit singular samples, a fixed-(theta,rho) phi slice, and a unit-scale factor-check table.")
    lines.append("")
    ...
    lines.append("")
    lines.append("A supplementary unit-scale factor check gives:")
    lines.append(f"- candidate-factor rows: {candidate_check['rows']}")
    lines.append(f"- max |detJ - cos(2phi)| at omega = 0: {candidate_check['max_abs_err']}")
    lines.append("")
    ...
    lines.append("")
    lines.append("The reference table `phi_reference_table.json` shows the alternating pattern detJ = {+1, 0, -1, 0, ...} exactly where cos(2phi) does.")
    lines.append("")
    lines.append("## Interpretation")
    lines.append("")
    lines.append("The wider angle domain does not change the core mapping result; it reveals the same singular architecture repeated across multiple periods.")
    lines.append("")
    lines.append("Omega rescales the determinant magnitude through e^(4 omega), while phi alone controls where the ordered chart loses angular rank.")
    lines.append("")
    lines.append("This is a chart-geometry result: it identifies where local angular independence fails, not yet a dynamical hierarchy of physical traits.")
    lines.append("")
...
def main() -> None:
    ...
    candidate_check = write_candidate_factor_check()
    phi_reference = write_phi_reference_table()
    write_summary(domain, rows, candidate_check, phi_reference)
