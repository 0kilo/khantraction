...
OUTDIR = ROOT / "solutions" / "phase_a" / "phase_a_role_stability_stress_test"
...
def write_summary(summary: Dict[str, object], omega_rows: List[Dict[str, float]]) -> None:
    ...
    lines.append("")
    lines.append("Generated by `analysis/phase_a/phase_a_role_stability_stress_test.py`.")
    lines.append("")
    lines.append("## Question")
    lines.append("")
    lines.append("- Does the emerging Phase A role picture survive a wider positive-omega range and a broader sample of the active angle box?")
    lines.append("")
    lines.append("## Method")
    lines.append("")
    lines.append("- Run a broad stress scan over omega in [0.1, 2.5] and coarse samples of theta, phi, and rho on the active box.")
    lines.append("- Track equality of angular tangent norms and the hierarchy of pairwise overlaps.")
    lines.append("- Perform a dedicated omega-scaling check at one fixed angular point.")
    lines.append("")
    ...
    lines.append("")
    lines.append("- The three angular channels remain equal in norm across the full sampled omega range; the largest mismatch is only floating-point noise.")
    lines.append("- Phi stays orthogonal to both theta and rho across the full stress scan, while the theta-rho overlap continues to span the full [-1, 1] range.")
    lines.append("- This shows the Phase A role picture is stable across positive omega, not an artifact of one local patch.")
