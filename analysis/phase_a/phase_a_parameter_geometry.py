...
OUTDIR = ROOT / "solutions" / "phase_a" / "phase_a_parameter_geometry"
...
def summarize_grid() -> Dict[str, object]:
    samples = [k * math.pi / 8.0 for k in range(-16, 17)]
    omega = 0.0
    rows = []
    min_abs_det = None
    min_point = None
...
def write_summary(summary: Dict[str, object], named: List[Dict[str, object]]) -> None:
    ...
    lines.append("")
    lines.append("Generated by `analysis/phase_a/phase_a_parameter_geometry.py`.")
    lines.append("")
    lines.append("## Question")
    lines.append("")
    lines.append("- Does the ordered map cleanly separate the scale coordinate from the angular geometry?")
    lines.append("- Away from singular slices, do theta, phi, and rho act as three genuinely distinct local angular directions?")
    lines.append("")
    lines.append("## Method")
    lines.append("")
    lines.append("- Evaluate the exact Jacobian from the ordered map on a coarse active-domain grid at fixed omega = 0.")
    lines.append("- Record determinant, rank, angular tangent norms, and pairwise angular cosines at every sampled point.")
    lines.append("- Inspect named benchmark points, including the current quaternion-rich sector guess.")
    lines.append("")
    lines.append("## Key results")
    lines.append("")
    lines.append(f"- Grid points scanned on the active domain: {summary['grid_points']}")
    lines.append(f"- Minimum |det J| on the coarse grid: {summary['min_abs_detJ']}")
    lines.append(f"- Max absolute cosine between any sampled angular tangent pair: {summary['max_abs_angular_cosine']}")
    lines.append(f"- Number of coarse special/singular points detected: {summary['special_point_count']}")
    lines.append("")
    lines.append("All named benchmark points remain rank 4 with equal angular tangent norms, so regular points do not privilege one angular channel by raw magnitude.")
    lines.append("")
    ...
    lines.append("")
    lines.append("- The ordered map separates scale cleanly: omega multiplies every component uniformly, while the angular geometry lives in the normalized quaternion state.")
    lines.append("- Away from singular slices, theta, phi, and rho are locally distinct tangent directions, so Phase A is justified in treating the angular channels symmetrically at the start.")
    lines.append("- The special points are chart singularities where local distinguishability drops; they do not imply that one angular channel disappears globally.")
