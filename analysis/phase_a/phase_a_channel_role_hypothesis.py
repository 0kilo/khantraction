...
OUTDIR = ROOT / "solutions" / "phase_a" / "phase_a_channel_role_hypothesis"
...
def write_summary(phi_control: Dict[str, object], theta_rho: Dict[str, object], phi_profile: List[Dict[str, float]]) -> None:
    ...
    lines.append("")
    lines.append("Generated by `analysis/phase_a/phase_a_channel_role_hypothesis.py`.")
    lines.append("")
    lines.append("## Question")
    lines.append("")
    lines.append("- Does phi behave like a peer angular direction, or does it act mainly as the controller of the theta-rho relation?")
    lines.append("")
    lines.append("## Method")
    lines.append("")
    lines.append("- Sweep phi broadly across the active box while sampling multiple theta-rho pairs.")
    lines.append("- Inspect a dense fixed-(theta,rho) phi profile to see how the overlap pattern evolves continuously.")
    lines.append("- Recheck regular phi values separately to confirm that the result is not only a singular-slice artifact.")
    lines.append("")
    ...
    lines.append("")
    phi_profile_min = min(row["cos_theta_rho"] for row in phi_profile)
    phi_profile_max = max(row["cos_theta_rho"] for row in phi_profile)
    lines.append(f"- dense fixed-pair phi profile: cos(theta,rho) ranges from {phi_profile_min} to {phi_profile_max}")
    lines.append("")
    ...
    lines.append("")
    lines.append("- Phi remains numerically orthogonal to both theta and rho across the sampled domain to machine precision.")
    lines.append("- The theta-rho overlap sweeps continuously across the full alignment range as phi varies, so the asymmetry is relational, not a norm imbalance.")
    lines.append("- This supports the Phase A mapping-level role picture: phi acts as the separator or mixing controller for a paired theta-rho subsystem.")
