...
1. **Create `analysis/phase_b_exact_radial_solver.py`**: Upgrade the matter-side equations to ingest the dynamically evaluated \(R = \frac{-\kappa T + 6\kappa\xi \square |q|^2}{1 + 2\kappa\xi |q|^2}\). This will require substituting \(\square |q|^2\) in static spherically symmetric coordinates.
...
3. **Write `solutions/phase_b_exact_radial_solver/` outputs.**